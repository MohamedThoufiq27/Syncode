...
const Sidebar = () => {
  ...
  const handleOpenSidebar = (name) => {
    setSidebarOpen({
      ...
      isVideoChat:(!sidebarOpen.isVideoChat ? name === "isVideoChat":false)
    })
  }
  const openChat = () => {
    setIsChatOpen(true);
  ...
  return (
    <div className='w-15 h-screen bg-gray-900 pt-2 overflow-visible'>
        ...
        <Tooltip text='Video Chat'>
            <div
              className={`absolute left-0 top-0 h-full w-1 rounded-r-sm transition-all duration-150
                ${sidebarOpen.isVideoChat ? "bg-linear-to-b from-pink-600 to-violet-500" : "bg-transparent"}`}
            />
            <button onClick={()=>handleOpenSidebar("isVideoChat")} className='cursor-pointer p-3  rounded-lg'>
              <IoVideocamOutline className='size-9 text-white' />
            </button>
        </Tooltip>
